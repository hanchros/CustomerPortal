...
exports.createTeamChat = async (req, res, next) => {
  try {
    let pms = await ProjectMember.find({ project: req.body.projectId });
    let pts = pms.map((pm) => {
      return pm.participant;
    });
    const conversation = new Conversation({
      participants: pts,
      name: req.body.name,
    ...
    let cv = await conversation.save();
    return res.status(200).json({ conversationId: cv._id });
  } catch (err) {
    return next(err);
  }
};
