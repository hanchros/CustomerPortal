...
exports.createPDFDoc = function (values, description) {
  const imgPath = `${__dirname}/../uploads/${values.invite}.png`;
  const pdfPath = `${__dirname}/../uploads/${values.invite}.pdf`;
  ...
  return new Promise((resolve, reject) => {
    download(values.logo, imgPath, function () {
      ...
      pdfDoc.moveDown(1);
      pdfDoc.fontSize(13).text(description, 100);
      pdfDoc.moveDown(1.5);
      pdfDoc
        .fontSize(15)
        .text(
          "This invitation is also the first time you have seen a blockchain authenticated smart document.  To see the power of the automation of authenticated documents go to the link below:",
          72
        );
      pdfDoc.moveDown(1);
...
exports.createSCPDFDoc = async (values) => {
  const imgPath = `${__dirname}/../uploads/${values.invite}.png`;
  const pdfPath = `${__dirname}/../uploads/${values.invite}.pdf`;
  const link = `https://www.collaborate.app/${encodeURIComponent(
    values.sender_organization
  ...
  return new Promise((resolve, reject) => {
    download(values.logo, imgPath, function () {
      ...
      pdfDoc.image(imgPath, { width: 150 });
      pdfDoc.moveDown(1.5);
      pdfDoc.fontSize(15).text(`Dear ${values.contact}`);
      pdfDoc.moveDown(1);
      pdfDoc.text(
        `You have been invited by ${values.sender_name} from ${values.sender_organization} to join collaboration application`
      );
      pdfDoc.moveDown(1.5);
      pdfDoc
        .fontSize(15)
